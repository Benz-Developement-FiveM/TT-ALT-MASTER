...
local function HasUsedSnowman(closestSnowman, citizenId)
    local callback = false
    for _, x in pairs(snowmans[closestSnowman].citizenIDs) do
            if citizenId == x then
                callback = true
                break
            end
...
RegisterServerEvent('rs-xmas:server:GetGift', function(closestTree)
    ...
    if not HasUsedTree(closestTree, citizenId) then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Received a present", 'success')
...
-- Get gifts from Snowman --
RegisterServerEvent('rs-xmas:server:GetGift', function(closestSnowman)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local citizenId = Player.PlayerData.citizenid
    local idk = 0
    ...
    if not HasUsedSnowman(closestSnowman, citizenId) then
        Player.Functions.AddItem('xmas_gift', Config.GiftAmount)
        TriggerClientEvent('inventory:client:ItemBox', src, sharedItems['xmas_gift'], "add", Config.GiftAmount)
        TriggerClientEvent("QBCore:Notify", src, "Received a present", 'success')
        table.insert(snowmans[closestSnowman].citizenIDs, citizenId)
    else
        TriggerClientEvent("QBCore:Notify", src, "There\'s no gifts at this Snowman with your name on it!", 'error')
    end
end)
...
RegisterServerEvent('rs-xmas:server:OpenGift', function(closestTree)
    ...
    if coalChance <= 8 then -- 80% Chance to get actual gifts
        ...
        for s = 1, #gifts do
            ...
            TriggerClientEvent("QBCore:Notify", src, "Received...", 'success')
            if idk == #gifts then
...
-- Create Snowmans Table w/ IDs --
RegisterServerEvent('rs-xmas:server:SnowmansTable', function()
    for s = 10, #Config.Snowmans.spawnLocations do
        snowmans[s] = {
            citizenIDs = {}
        }
    end
...
AddEventHandler('onResourceStart', function(resource)
    if resource == GetCurrentResourceName() then
        ...
        TriggerEvent('rs-xmas:server:SnowmansTable')
