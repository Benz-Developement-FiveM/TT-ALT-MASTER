...
-- Event
RegisterServerEvent('boii-weed:sv:Trim',function(strain, returnitem, bagamount)
    local pData = Core.Functions.GetPlayer(source)
    local DrugXP = pData.PlayerData.metadata[MetaDataName]
    if strain == 'Skunk' then
        pData.Functions.AddItem(returnitem, bagamount)
        TriggerClientEvent('inventory:client:ItemBox', source, Core.Shared.Items[returnitem], 'add', bagamount)
        if Config.XP.Use then
            pData.Functions.SetMetaData(MetaDataName, (DrugXP + math.random(1,3))) -- Edit xp reward here
            TriggerClientEvent('boii-weed:notify', source, Language.Trimming['trimxp'], 'success')
        else
            TriggerClientEvent('boii-weed:notify', source, Language.Trimming['trimnoxp'], 'success')
        end
        return 
    end
    ...
    if strain == 'White Widow' then
        pData.Functions.AddItem(returnitem, bagamount) -- Change return item here
        TriggerClientEvent('inventory:client:ItemBox', source, Core.Shared.Items[returnitem], 'add', bagamount)
        if Config.XP.Use then
            pData.Functions.SetMetaData(MetaDataName, (DrugXP + math.random(1,3))) -- Edit xp reward here
            TriggerClientEvent('boii-weed:notify', source, Language.Trimming['trimxp'], 'success')
