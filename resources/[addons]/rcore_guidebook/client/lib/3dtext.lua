            ...
            for i, self in pairs(nearTextsV2) do
                if self.rendering and not self.destroyed then
                    ...
                    if gameName == GameType.REDM then
                        local onScreen, _x, _y = GetScreenCoordFromWorldCoord(self.position.x, self.position.y,
                            self.position.z)
                        local str = CreateVarString(10, "LITERAL_STRING", self.text, Citizen.ResultAsLong())
                        if onScreen then
                            ...
                            local factor = (string.len(self.text)) / 225
                            DrawSprite("feeds", "hud_menu_4a", _x, _y + 0.0125, 0.015 + factor, 0.03, 0.1, 35, 35, 35,
                                190, 0)
