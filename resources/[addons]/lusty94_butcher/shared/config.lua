...
--Blip Settings
Config.Blips = {
    {title = 'Job:Cluckin Bell Butchers', colour = 5, id = 310, coords = vector3(-73.59, 6267.08, 31.32), scale = 0.7, useblip = true}, -- BLIP FOR CHICKEN FACTORY
}
...
Config.CoreSettings = {
    ...
    Target = {
        Type = 'qb', -- target script name support for qb-target and qb-target 
        --EDIT TARGETS.LUA TO ADD YOUR OWN CUSTOM TARGET SCRIPT        
        --use 'qb' for qb-target
        --use 'ox' for qb-target
...
Config.InteractionLocations = {
    --name must be unique, size is for qb-target only, job must match job in core and in Cofig.CoreSettings.Job.Name
    JobAreas = {
...
Config.Selling = {
    ...
    PedModel =  's_m_m_ammucountry', -- name of ped model
