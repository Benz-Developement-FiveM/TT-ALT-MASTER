function gameFlappy()
{
    ...
    SFX_flappyScore.volume = 0.1;
    SFX_FLAP.volume = 0.01
    SFX_COLLISION.volume = 0.1;
    SFX_FALL.volume = 0.1;
    SFX_SWOOSH.volume = 0.1;


    cvsFlappy = document.getElementById('game')
    ctx = cvsFlappy.getContext('2d')
    ...
    frame = 0;
    degree = Math.PI/180
    SFX_flappyScore.src = 'audio/sfx_point.wav'
            ...
            if (gameState.current == gameState.play) {
                ...
                for (let i = 0; i < this.pipeGenerator.length; i++) {
                    ...
                    if (b.left < p.right &&
                        ...
                        b.top < p.top.bottom &&
                        b.bottom > p.top.top) {
                        gameState.current = gameState.flappyGameOver
                        SFX_COLLISION.play()
                    }
...
//ground floor
    ground = {
        ...
        //ANIMATION:  ground scrolls to the left in a continuous flappyLoop when game state is at play
...
//current flappyScore, top flappyScore, tracker
    flappyScore = {
        current: 0,
        ...
        //display the flappyScore
        render: function() {
            if (gameState.current == gameState.play ||
                gameState.current == gameState.flappyGameOver) {
                //change current flappyScore number value to string value and access each place value
                ...
                //if current flappyScore has thousands place value: the game is over
                if (this.current >= 1000) {
                    gameState.current = gameState.flappyGameOver

                    //if current flappyScore has ones, tens, and hundreds place value only
                    ...
                    //if current flappyScore has ones and tens place value only
                    ...
                    //if current flappyScore has ones place value only
                } else {
    ...
    bird = {
        ...
        position: function() {
            if (gameState.current == gameState.getReady) {
                ...
                if (this.y+this.h/2 >= cvsFlappy.height-ground.h) {
                    this.y = cvsFlappy.height-ground.h - this.h/2
...
//get ready screen
    getReady = {
        ...
        height: 160,
        //values for drawing on canvas
        x: cvsFlappy.width/2 - 174/2,
        y: cvsFlappy.height/2 - 160,
        w: 174,
        h: 160,
        //object's render function that utilizes all above values to draw image onto canvas
        render: function() {
...
//game over screen
    flappyGameOver = {
        ...
        render: function() {
            ...
            if (gameState.current == gameState.flappyGameOver) {
                ctx.drawImage(theme1, this.imgX,this.imgY,this.width,this.height, this.x,this.y,this.w,this.h)
...
//anything to be drawn on canvas goes in here
    let draw = () => {
        ...
        ctx.fillRect(0,0, cvsFlappy.width,cvsFlappy.height)
        //things to draw
        bg.render()
        pipes.render()
        ground.render()
...
//game flappyLooper
    ...
    flappyLoop()
    setInterval(flappyLoop, 17)

    /*************************
     ***** EVENT HANDLERS *****
     *************************/
//on mouse click // tap screen
    cvsFlappy.addEventListener('click', () => {
