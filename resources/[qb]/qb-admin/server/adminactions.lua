...
AdminPanel.OnlineBanPlayerFromDiscord = function(player, time, reason)
    ...
    if Config.AnnounceBan then
        TriggerClientEvent("chat:addMessage", -1, {
            template = "<div class=\"chat-message server\"><strong>{0}</strong> has been banned by discord admin command for {2} hours. Reason: {3}</div>",
            args = {GetPlayerName(player), time / 60 / 60, reason}
        })
    end
...
RegisterServerEvent("919-admin:server:DeleteAllEntities", function(entityType)
    ...
    if entityType == 1 then
        if not AdminPanel.HasPermission(src, "massdv") then
            return
        end
    elseif entityType == 2 then
        entityTypeString = "PEDS"
        if not AdminPanel.HasPermission(src, "massdp") then
            return
        end
    end
...
RegisterServerEvent("919-admin:server:GiveItem", function(targetId, item, amount)
    ...
    if AdminPanel.HasPermission(src, "giveitem") then
        ...
        local itemList = Compat.GetItemsList()
        ...
        elseif Config.Framework == "qbox" then
            if itemList[item] ~= nil then
                exports[Config.OxInventoryResourceName]:AddItem(targetId, item, amount)
                TriggerEvent("qb-log:server:CreateLog", "adminactions", "Item Given", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** gave " .. item .. " (x" .. amount .. ") to " .. GetPlayerName(targetId), false)
                TriggerClientEvent("919-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.gaveItem", {value = item}))
                TriggerClientEvent("QBCore:Notify", targetId, Lang:t("notify.givenItem", {value = item}), "success")
            else
...
AdminPanel.RemoveItemFromDiscord = function(targetId, item, amount)
    ...
    if Config.Framework == "qbcore" then
        if itemList[item] ~= nil then
            ...
            targetPlayer = QBCore.Functions.GetPlayer(tonumber(targetId))
            if targetPlayer then
                targetPlayer.Functions.RemoveItem(item, amount)
                ...
                TriggerClientEvent("QBCore:Notify", targetId, Lang:t("notify.removedItem", {value = item}), "error")
                return true, ""
            end
        else
            return false, "invalid item"
    ...
    elseif Config.Framework == "qbox" then
        if itemList[item] ~= nil then
            exports[Config.OxInventoryResourceName]:RemoveItem(targetId, item, amount)
            TriggerEvent("qb-log:server:CreateLog", "adminactions", "Item Removed", "red", "**DISCORD** removed " .. item .. " (x" .. amount .. ") from " .. GetPlayerName(targetId), false)
            TriggerClientEvent("QBCore:Notify", targetId, Lang:t("notify.removedItem", {value = item}), "error")
            return true, ""
        else
    ...
    elseif Config.Framework == "esx" then
        if itemList[item] ~= nil then
            if not Config.IsUsingOverextendedInventory then
                local targetPlayer = ESX.GetPlayerFromId(tonumber(targetId))
                if targetPlayer then
                    targetPlayer.removeInventoryItem(item, amount)
                    TriggerEvent("qb-log:server:CreateLog", "adminactions", "Item Removed", "red", "**DISCORD** removed " .. item .. " (x" .. amount .. ") from " .. GetPlayerName(targetId), false)
                    TriggerClientEvent("QBCore:Notify", targetId, Lang:t("notify.removedItem", {value = item}), "error")
                    return true, ""
                end
...
elseif Config.Framework == "esx" then
    ...
    ESX.RegisterServerCallback("919-admin:server:GetPlayerPositions", function(source, cb)
        ...
        for k, v in pairs(ESX.GetPlayers()) do
            ...
            Player = ESX.GetPlayerFromId(tonumber(v))
