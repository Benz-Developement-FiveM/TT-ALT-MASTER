        ...
        for _, playerId in ipairs(GetPlayers()) do
            ...
            if charInfo.CharacterIdentifier == data.name then
                if Config.Framework == "qbcore" or Config.Framework == "qbox" then
                    TriggerClientEvent("QBCore:Notify", playerId, {text = "Report Reply", caption = data.message}, "info", 10000)
                elseif Config.Framework == "esx" then
                    ESX.GetPlayerFromId(playerId).showNotification("Report Reply: "..data.message, "info", 10000)
                end
            end
        end
    end
...
Compat.CreateCallback("qb-admin:server:GetPlayerCountHistory", function(source, cb)
            ...
            table.insert(playerList,
                {
                    ...
                    health = GetEntityHealth(GetPlayerPed(playerId)) - 100,
...
RegisterNetEvent("919-admin:server:RequestMapInfo", function()
    ...
    if AdminPanel.HasPermission(src, "mapinfo") then
        ...
        for _,strId in pairs(GetPlayers()) do
            ...
            local playerData = Compat.GetCharacterData(playerId)
...
AdminPanel.GetRole = function(targetId)
    ...
    if Player then
        for k,v in ipairs(Config.RoleOrder) do
            if Config.Framework == "qbcore" or Config.Framework == "qbox" then
                if QBCore.Functions.HasPermission(targetId, v) then
                    return v
                end
            elseif Config.Framework == "esx" then
