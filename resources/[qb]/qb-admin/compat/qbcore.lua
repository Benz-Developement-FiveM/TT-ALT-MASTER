if Config.Framework == "qbcore" then
    ...
    if GetResourceState('qbx_core') == "started" then
        print("[^1ERROR^7] ^3QBox^7 is installed. Please change Config.FrameworkResourceName to 'qbox' in config.lua, as the qb-core compatibility layer is not fully compatible.")
        return
    end
else
    return
    ...
    Compat = {
        ...
        GetOfflinePlayerFromCharacterIdentifier = function(CitizenId)
            ...
            local MenuPlayerData = {
                ...
                bank = ("$%s"):format(comma_value(PlayerData.money.bank)), cash = ("$%s"):format(comma_value(PlayerData.money.cash)), steamid = res.license, citizenid = res.citizenid,
                ...
                thirst = PlayerData.metadata['thirst'], injail = PlayerData.metadata['injail'], lastonline = res.last_updated, amountofVehicles = amountofVehicles[1].count, job = PlayerData.job.label,
                    ...
                    table.insert(PlayerList,
                        {
                            ...
                            health = GetEntityHealth(GetPlayerPed(Player.PlayerData.source)) - 100,
